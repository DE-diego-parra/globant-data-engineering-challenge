        ...
        try:
            if request.table == "employees":
                ...
                for record in valid_records:
                    ...
                    values_list.append(
                        f"({record['id']}, '{name_escaped}', '{record['datetime']}', {record['department_id']}, {record['job_id']})"
                    )
                
                insert_query = f"""
                INSERT INTO `{PROJECT_ID}.{DATASET_ID}.employees` (id, name, datetime, department_id, job_id)
                VALUES {', '.join(values_list)}
                """
                
            elif request.table == "departments":
                values_list = []
                for record in valid_records:
                    dept_escaped = record['department'].replace("'", "''")
                    values_list.append(f"({record['id']}, '{dept_escaped}')")
                
                insert_query = f"""
                INSERT INTO `{PROJECT_ID}.{DATASET_ID}.departments` (id, department)
                VALUES {', '.join(values_list)}
                """
                
            elif request.table == "jobs":
                values_list = []
                for record in valid_records:
                    job_escaped = record['job'].replace("'", "''")
                    values_list.append(f"({record['id']}, '{job_escaped}')")
                
                insert_query = f"""
                INSERT INTO `{PROJECT_ID}.{DATASET_ID}.jobs` (id, job)
                VALUES {', '.join(values_list)}
                """
